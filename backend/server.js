...
// ─── CORS Middleware (per richieste HTTP da itch.io) ──────────
...
// ─── Keep-Alive Ping (Render free tier anti-sleep) ────────────
app.get('/ping', (req, res) => {
    res.json({ status: 'ok', time: new Date().toISOString() });
});

app.get('/', (req, res) => {
    res.json({ status: 'Wordle Multiplayer Backend running on Render.com' });
});
